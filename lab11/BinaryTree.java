...
public class BinaryTree implements Dictionary {
  ...
  private BinaryTreeNode findHelper(Comparable key, BinaryTreeNode node) {
	  BinaryTreeNode walker = node;
	  while (walker != null) {
		  if (key.compareTo(walker.entry.key()) == 0) {
			  return walker;
		  }
		  else if (key.compareTo(walker.entry.key()) < 0) {
  ...
  public Entry remove(Object key) {
    // Replace the following line with your solution.
	  ...
	  BinaryTreeNode walker = findHelper((Comparable) key, root);
	  BinaryTreeNode pp = findHelper((Comparable) key, root).parent;
	  ...
	  // case root
		  ...
		  else if ( root.leftChild!=null && root.rightChild == null) {//1 child
			  root = root.leftChild;
			  root.parent = null;
			  this.size--;
		  }
		  else if ( root.leftChild ==null && root.rightChild != null) {//1child
			  root = root.rightChild;
			  root.parent = null;
			  this.size--;
		  }
		  else if ( root.leftChild !=null && root.rightChild != null) {//1child
			  BinaryTreeNode x = root.rightChild; // subtree x < walker's right child
		        while (x.leftChild!=null) // return the minimum value
			    x = x.leftChild;
		        
		        
		        walker.entry = x.entry; // replace the minimum entry with walker
		       
		       //remove x (suppose x only has right child or no child)
	  ...
	  // case child
	      ...
	      if (walker.leftChild == null && walker.rightChild == null) { //no child
               if (((Comparable) key).compareTo(pp.entry.key()) <= 0) { // key < parent, remove left key
            	   pp.leftChild = null;
               }
               else { // key> parent, remove right key
            	   pp.rightChild = null;
               }
             this.size--;
	      }     
	      else if ( walker.leftChild!=null && walker.rightChild == null) {//1 child
              if (((Comparable) key).compareTo(pp.entry.key()) <= 0) { // key < parent, remove left key
           	   pp.leftChild = walker.leftChild;
              }
              else { // key> parent, remove right key
           	   pp.rightChild = walker.leftChild;
              }
		     this.size--;
	      
	      }
	      else if ( walker.leftChild ==null && walker.rightChild != null) {//1child
		      if (((Comparable) key).compareTo(pp.entry.key()) <= 0) { // key < parent, remove left key
	           	   pp.leftChild = walker.rightChild;
	              }
	              else { // key> parent, remove right key
	           	   pp.rightChild = walker.rightChild;
	              }
			  this.size--;
		      
	      }
	      else if ( walker.leftChild !=null && walker.rightChild != null) {//2child
		        BinaryTreeNode x = walker.rightChild; // subtree x < walker's right child
		        while (x.leftChild!=null) {// return the minimum value
			    x = x.leftChild;
		       ...
		       //remove x (suppose x only has right child or no child)
			    ...
			    walker.rightChild = null;
		        }
		        else if (x.leftChild==null && x.rightChild!=null) {
		        walker.rightChild = x.rightChild;
