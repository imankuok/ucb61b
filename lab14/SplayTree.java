...
public class SplayTree implements Dictionary {
  ...
  private void splayNode(BinaryTreeNode node) {
    ...
    // with your solution.
    while (node.parent != null) { // when root == node, stop
    	if (node.parent.parent == null) { // root
    		zig(node);
    	}
    	else{
    		if (node == node.parent.leftChild && node.parent == node.parent.parent.leftChild || node == node.parent.rightChild && node.parent == node.parent.parent.rightChild) {
    		    zigZig(node);
    		}
    		else {
    			zigZag(node);
