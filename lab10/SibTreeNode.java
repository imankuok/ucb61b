...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART II HERE
	  ...
	  newkid.parent = this;
           ...
           SibTreeNode walker = this.firstChild;
           if (walker == null) {
               this.firstChild = newkid;}
           else {
	        while (( walker.nextSibling != null) && (c > 2)) {
	        walker = walker.nextSibling;
	        c--;}
	
		  } newkid.nextSibling = walker.nextSibling;
		  walker.nextSibling = newkid;
  ...
  public void removeLeaf() throws InvalidNodeException {
    // FILL IN YOUR SOLUTION TO PART III HERE.
  	int n = this.parent.children();
  	SibTreeNode walker = this.parent.firstChild;
	  ...
	  if(this.isValidNode()){
			   ...
			   if (this.firstChild == null) {
				   ...
				   if (this.nextSibling ==null ) { // when 1 child 
					   walker = null;
				   }
			       else if (walker.nextSibling == this) { // when 2 child
	                	walker = walker.nextSibling;
	                }
	               else {// child >3

	         	        while (( walker.nextSibling != null) && (n > 3)) {
	         	        walker = walker.nextSibling;
	         	        n--;}
	         	        
	         	        walker.nextSibling = walker.nextSibling.nextSibling;}
